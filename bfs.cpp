...
std::optional<uint16_t> bfs(std::span<const std::vector<uint8_t>> maze,
                               ...
                               const std::pair<std::size_t, std::size_t>& finish)
{
    ...
    std::vector<std::vector<bool>> visited(n, std::vector<bool>(m, -1));
    ...
    while (!q.empty())
    {
        ...
        for (std::size_t i = 0; i < 4; ++i)
        {
            ...
            if (nx < m && ny < n && !visited[ny][nx] && maze[nx][ny] != 1)
            {
                visited[ny][nx] = true;
                q.emplace(nx, ny);
            }
